...
fn plugin_install_direct_from_git_succeeds() {
    ...
    assert!(root.join("plugins").join("remote-node-plugin").join("config.toml").is_file());
...
fn plugin_install_build_required_from_git_succeeds() {
    ...
    let artifact = root.join("plugins").join("built-plugin").join("bin").join("generated.sh");
...
fn plugin_install_force_replaces_existing_target() {
    ...
    let first = Command::new(chainbot_bin())
        .env("CHAINBOT_CONFIG_DIR", &root)
        .args(["plugin", "install", "git", repo_a.to_string_lossy().as_ref()])
        .output()
        .expect("first install should execute");
    assert!(first.status.success());
    ...
    let artifact = root.join("plugins").join("force-plugin").join("bin").join("plugin.sh");
    let contents = fs::read_to_string(artifact).expect("artifact should be readable");
...
fn basic_root() -> PathBuf {
    workspace_root().join("target").join("test-roots").join("basic-plugin-install")
}
