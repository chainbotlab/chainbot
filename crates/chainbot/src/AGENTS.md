...
- `config.rs`: Explicit root-layout resolver plus TOML definition loaders for root config/workflows/triggers/plugins with version validation.
- `workflow.rs`: Workflow semantic contract with DAG/cycle validation, typed runtime variable namespaces and precedence layers, explicit subflow input/output boundaries, and deterministic `when` evaluators.
- `trigger.rs`: Trigger-plane runtime that validates builtin/external trigger sources, host policy, dedup/cooldown coordination, restart-safe duplicate suppression, and normalized run-request emission.
...
- `plugin.rs`: Versioned plugin manifest contract, external node-plugin host boundary, and executable/protocol/schema validation helpers.
