...
fn run_build(
    ...
    let workdir = plugin
        ...
        .ok_or_else(|| ContractError::CliUsage {
            message: format!("plugin {} build workdir could not be resolved", plugin.plugin_id),
        })?;
    let mut command = Command::new(&build.command[0]);
    if build.command.len() > 1 {
        command.args(&build.command[1..]);
    }
    let output = command.current_dir(&workdir).output().map_err(|source| ContractError::CliUsage {
        message: format!("failed to start build command for plugin {}: {source}", plugin.plugin_id),
    })?;
    ...
    for (from, to) in plugin.source_manifest.output_paths(package_root, workspace_root)? {
...
fn validate_artifact(package_root: &Path, plugin: &DiscoveredPlugin) -> Result<(), ContractError> {
    ...
    match plugin.source_manifest.runtime {
        SourceRuntime::Python => validate_script_artifact(&artifact_path, "python", &plugin.plugin_id)?,
...
fn validate_script_artifact(
    ...
    let shebang = read_shebang(path)?.ok_or_else(|| ContractError::CliUsage {
        message: format!("plugin {plugin_id} entry artifact is missing a shebang: {}", path.display()),
    })?;
...
fn validate_executable_artifact(path: &Path, plugin_id: &str) -> Result<(), ContractError> {
    #[cfg(unix)]
    {
        ...
        if mode & 0o111 == 0 {
            return Err(ContractError::CliUsage {
                message: format!("plugin {plugin_id} entry artifact is not executable: {}", path.display()),
            });
        }
    }
    Ok(())
