...
async fn handle_request_json_inner(input: &str) -> Result<String, PluginError> {
    ...
    match jsonrpc_id {
        Some(id) => serde_json::to_string(&contract::JsonRpcResponse::success(id, response)).map_err(PluginError::from),
        None => serde_json::to_string(&response).map_err(PluginError::from),
    }
}

pub fn failure_response(message: &str) -> String {
    serde_json::json!({"contract_version": "1.0.0", "success": false, "error": message}).to_string()
}
...
pub fn failure_jsonrpc_response(id: contract::JsonRpcId, message: &str) -> String {
    serde_json::to_string(&contract::JsonRpcResponse::failure(id, message)).unwrap_or_else(|_| failure_response(message))
}
