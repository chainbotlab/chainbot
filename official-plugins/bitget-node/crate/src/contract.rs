...
impl PluginRequest {
    ...
    pub fn activation_secret(&self, key: &str) -> Option<&str> {
        self.activation.as_ref()?.secrets.get(key).map(String::as_str)
    }
...
impl PluginResponse {
    pub fn success(output: BTreeMap<String, Value>, result_state: Option<&'static str>) -> Self {
        Self { contract_version: String::from("1.0.0"), success: true, result_state, output, error: None }
    }
}
...
impl JsonRpcResponse {
    pub fn success(id: JsonRpcId, response: PluginResponse) -> Self {
        Self {
            ...
            id,
            result: Some(JsonRpcSuccessResult { contract_version: "1.0.0", result_state: response.result_state, output: response.output }),
            error: None,
        }
    }
