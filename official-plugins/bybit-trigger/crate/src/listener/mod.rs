...
pub async fn run_listener_with_writer(
    ...
    if source != "bybit_market_stream" && source != "bybit_user_stream" {
        return Err(format!("unsupported Bybit trigger source {source}"));
    }
    ...
    match source {
        "bybit_market_stream" => market_stream::run_market_listener(&command, stdout).await,
        "bybit_user_stream" => user_stream::run_user_stream_listener(&command, stdout).await,
