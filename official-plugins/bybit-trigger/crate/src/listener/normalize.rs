...
pub fn listener_kind(source: &str) -> &'static str {
    match source {
        "bybit_market_stream" => "market_stream",
        "bybit_user_stream" => "user_stream",
        _ => "event_stream",
    }
}

pub fn event_stream_name(command: &TriggerStartCommand, payload: &Value, requested_streams: Option<&[String]>) -> String {
    ...
    if command.source == "bybit_user_stream" {
        return String::from("order");
    }
...
pub fn event_time_ms(payload: &Value) -> Option<i64> {
    payload
        ...
        .or_else(|| payload.get("creationTime").and_then(Value::as_i64))
        .or_else(|| payload.get("data").and_then(|data| data.get("T")).and_then(Value::as_i64))
        .or_else(|| payload.get("data").and_then(|data| data.get("updatedTime")).and_then(Value::as_str).and_then(|value| value.parse::<i64>().ok()))
}
...
pub fn event_key(command: &TriggerStartCommand, stream: &str, payload: &Value) -> Result<String, String> {
    ...
    let discriminator = first_string(event, &["id", "orderId", "orderLinkId", "symbol", "topic"])
        .or_else(|| first_string(payload, &["id", "topic", "type"]))
        .or_else(|| first_number(event, &["execId", "seq", "T"]))
...
pub fn is_subscription_ack(payload: &Value) -> bool {
    payload.get("op").and_then(Value::as_str) == Some("subscribe")
        && payload.get("success").and_then(Value::as_bool) == Some(true)
}

pub fn is_text_ping(payload: &Value) -> bool {
    payload.get("op").and_then(Value::as_str) == Some("ping")
}

pub fn text_pong() -> Value {
    json!({ "op": "pong" })
}
...
pub fn stream_error(payload: &Value) -> Option<(Option<i64>, String)> {
    ...
    if success == Some(false) {
        let code = payload.get("retCode").and_then(Value::as_i64);
        let message = payload
            .get("retMsg")
            .or_else(|| payload.get("message"))
            .and_then(Value::as_str)
            .map(str::to_owned)
            .unwrap_or_else(|| String::from("stream request failed"));
