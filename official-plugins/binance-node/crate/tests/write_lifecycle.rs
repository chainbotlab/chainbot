use axum::{http::StatusCode, routing::{delete, get, post}, Json, Router};
use binance_node_official_plugin::handle_request_json;
use serde_json::{json, Value};
use std::net::SocketAddr;
use tokio::net::TcpListener;

#[tokio::test]
#[serial_test::serial]
async fn binance_place_order_submit_only_returns_submitted_state() {
    let _loopback = LoopbackGuard::set();
    let server = TestBinanceServer::spawn().await;
    let response = handle_request_json(&json!({
        "contract_version": "1.0.0",
        "plugin_id": "binance-node",
        "node_id": "node-1",
        "operation": "binance_place_order",
        "input": {
            "product_line": "spot",
            "symbol": "BTCUSDT",
            "side": "BUY",
            "type": "MARKET",
            "quantity": "1",
            "confirmation_mode": "submit_only",
            "base_url": server.base_url
        },
        "activation": {
            "allowed_origins": [server.origin()],
            "secrets": {
                "api_key": "test-key",
                "api_secret": "test-secret"
            }
        }
    })
    .to_string())
    .await
    .expect("request should succeed");

    let payload: Value = serde_json::from_str(&response).expect("response should decode");
    assert_eq!(payload["success"], true);
    assert_eq!(payload["result_state"], "submitted");
    assert_eq!(payload["output"]["status"], "submitted");
}

#[tokio::test]
#[serial_test::serial]
async fn binance_place_order_safe_returns_submitted_for_non_terminal_order() {
    let _loopback = LoopbackGuard::set();
    let server = TestBinanceServer::spawn_with_query_status("NEW").await;
    let response = handle_request_json(&json!({
        "contract_version": "1.0.0",
        "plugin_id": "binance-node",
        "node_id": "node-2",
        "operation": "binance_place_order",
        "input": {
            "product_line": "spot",
            "symbol": "BTCUSDT",
            "side": "BUY",
            "type": "LIMIT",
            "quantity": "1",
            "price": "100",
            "time_in_force": "GTC",
            "base_url": server.base_url
        },
        "activation": {
            "allowed_origins": [server.origin()],
            "secrets": {
                "api_key": "test-key",
                "api_secret": "test-secret"
            }
        }
    })
    .to_string())
    .await
    .expect("request should succeed");

    let payload: Value = serde_json::from_str(&response).expect("response should decode");
    assert_eq!(payload["success"], true);
    assert_eq!(payload["result_state"], "submitted");
    assert_eq!(payload["output"]["status"], "submitted");
    assert_eq!(payload["output"]["metadata"]["terminal"], false);
}

#[tokio::test]
async fn binance_get_positions_rejects_spot_product_line() {
    let response = handle_request_json(&json!({
        "contract_version": "1.0.0",
        "plugin_id": "binance-node",
        "node_id": "node-3",
        "operation": "binance_get_positions",
        "input": {
            "product_line": "spot",
            "base_url": "http://127.0.0.1:1"
        },
        "activation": {
            "secrets": {
                "api_key": "test-key",
                "api_secret": "test-secret"
            }
        }
    })
    .to_string())
    .await
    .expect("request should return a failure response");

    let payload: Value = serde_json::from_str(&response).expect("response should decode");
    assert_eq!(payload["success"], false);
    assert!(payload["error"]
        .as_str()
        .unwrap_or_default()
        .contains("positions are unavailable for spot"));
}

#[tokio::test]
#[serial_test::serial]
async fn binance_place_order_safe_returns_settled_for_terminal_order() {
    let _loopback = LoopbackGuard::set();
    let server = TestBinanceServer::spawn_with_query_status("FILLED").await;
    let response = handle_request_json(&json!({
        "contract_version": "1.0.0",
        "plugin_id": "binance-node",
        "node_id": "node-4",
        "operation": "binance_place_order",
        "input": {
            "product_line": "spot",
            "symbol": "BTCUSDT",
            "side": "BUY",
            "type": "LIMIT",
            "quantity": "1",
            "price": "100",
            "time_in_force": "GTC",
            "base_url": server.base_url
        },
        "activation": {
            "allowed_origins": [server.origin()],
            "secrets": {
                "api_key": "test-key",
                "api_secret": "test-secret"
            }
        }
    })
    .to_string())
    .await
    .expect("request should succeed");

    let payload: Value = serde_json::from_str(&response).expect("response should decode");
    assert_eq!(payload["success"], true);
    assert_eq!(payload["result_state"], "settled");
    assert_eq!(payload["output"]["status"], "confirmed");
    assert_eq!(payload["output"]["metadata"]["terminal"], true);
}

#[tokio::test]
#[serial_test::serial]
async fn binance_place_order_safe_returns_pending_confirmation_when_query_fails() {
    let _loopback = LoopbackGuard::set();
    let server = TestBinanceServer::spawn_with_query_status("ERROR").await;
    let response = handle_request_json(&json!({
        "contract_version": "1.0.0",
        "plugin_id": "binance-node",
        "node_id": "node-4b",
        "operation": "binance_place_order",
        "input": {
            "product_line": "spot",
            "symbol": "BTCUSDT",
            "side": "BUY",
            "type": "LIMIT",
            "quantity": "1",
            "price": "100",
            "time_in_force": "GTC",
            "base_url": server.base_url
        },
        "activation": {
            "allowed_origins": [server.origin()],
            "secrets": {
                "api_key": "test-key",
                "api_secret": "test-secret"
            }
        }
    })
    .to_string())
    .await
    .expect("request should succeed");

    let payload: Value = serde_json::from_str(&response).expect("response should decode");
    assert_eq!(payload["success"], true);
    assert_eq!(payload["result_state"], "submitted");
    assert_eq!(payload["output"]["status"], "pending_confirmation");
    assert_eq!(payload["output"]["metadata"]["terminal"], false);
}

#[tokio::test]
async fn binance_keepalive_requires_listen_key() {
    let response = handle_request_json(&json!({
        "contract_version": "1.0.0",
        "plugin_id": "binance-node",
        "node_id": "node-5",
        "operation": "binance_keepalive_user_stream",
        "input": {
            "product_line": "spot"
        },
        "activation": {
            "allowed_origins": ["https://api.binance.com"],
            "secrets": {
                "api_key": "test-key"
            }
        }
    })
    .to_string())
    .await
    .expect("request should serialize failure response");

    let payload: Value = serde_json::from_str(&response).expect("response should decode");
    assert_eq!(payload["success"], false);
    assert!(payload["error"].as_str().unwrap_or_default().contains("listen_key is required"));
}

#[tokio::test]
#[serial_test::serial]
async fn binance_signed_requests_require_allowlisted_origin() {
    let _loopback = LoopbackGuard::set();
    let server = TestBinanceServer::spawn_with_query_status("FILLED").await;
    let response = handle_request_json(&json!({
        "contract_version": "1.0.0",
        "plugin_id": "binance-node",
        "node_id": "node-6",
        "operation": "binance_get_balances",
        "input": {
            "product_line": "spot",
            "base_url": server.base_url
        },
        "activation": {
            "allowed_origins": ["https://api.binance.com"],
            "secrets": {
                "api_key": "test-key",
                "api_secret": "test-secret"
            }
        }
    })
    .to_string())
    .await
    .expect("request should serialize failure response");

    let payload: Value = serde_json::from_str(&response).expect("response should decode");
    assert_eq!(payload["success"], false);
    assert!(payload["error"]
        .as_str()
        .unwrap_or_default()
        .contains("is not allowlisted"));
}

#[tokio::test]
async fn binance_rejects_mismatched_plugin_id() {
    let response = handle_request_json(&json!({
        "contract_version": "1.0.0",
        "plugin_id": "http-node",
        "node_id": "node-7",
        "operation": "binance_get_server_time",
        "input": {
            "product_line": "spot"
        }
    })
    .to_string())
    .await
    .expect("request should serialize failure response");

    let payload: Value = serde_json::from_str(&response).expect("response should decode");
    assert_eq!(payload["success"], false);
    assert!(payload["error"].as_str().unwrap_or_default().contains("plugin_id must be binance-node"));
}

struct TestBinanceServer {
    base_url: String,
}

impl TestBinanceServer {
    async fn spawn() -> Self {
        Self::spawn_with_query_status("FILLED").await
    }

    async fn spawn_with_query_status(query_status: &'static str) -> Self {
        let app = Router::new()
            .route("/api/v3/order", post(handle_place_order).get(handle_get_order).delete(handle_place_order))
            .route("/api/v3/openOrders", delete(handle_cancel_all))
            .route("/api/v3/account", get(handle_account))
            .with_state(query_status);
        let listener = TcpListener::bind("127.0.0.1:0")
            .await
            .expect("listener should bind");
        let address: SocketAddr = listener.local_addr().expect("listener should have addr");
        tokio::spawn(async move {
            axum::serve(listener, app).await.expect("server should run");
        });
        Self {
            base_url: format!("http://{address}"),
        }
    }

    fn origin(&self) -> String {
        self.base_url.clone()
    }
}

async fn handle_place_order() -> Json<Value> {
    Json(json!({
        "symbol": "BTCUSDT",
        "orderId": 777,
        "clientOrderId": "client-777",
        "status": "NEW"
    }))
}

async fn handle_get_order(axum::extract::State(query_status): axum::extract::State<&'static str>) -> impl axum::response::IntoResponse {
    if query_status == "ERROR" {
        return (StatusCode::BAD_GATEWAY, Json(json!({"code": -1000, "msg": "upstream unavailable"})));
    }
    (
        StatusCode::OK,
        Json(json!({
            "symbol": "BTCUSDT",
            "orderId": 777,
            "clientOrderId": "client-777",
            "status": query_status
        })),
    )
}

async fn handle_cancel_all() -> Json<Value> {
    Json(json!([
        {"symbol": "BTCUSDT", "orderId": 777, "status": "CANCELED"}
    ]))
}

async fn handle_account() -> Json<Value> {
    Json(json!({"balances": []}))
}

struct LoopbackGuard;

impl LoopbackGuard {
    fn set() -> Self {
        unsafe {
            std::env::set_var("CHAINBOT_HTTP_NODE_ALLOW_LOOPBACK_FOR_TESTS", "1");
            std::env::set_var("CHAINBOT_INTERNAL_ALLOW_TEST_DESTINATIONS", "1");
        }
        Self
    }
}

impl Drop for LoopbackGuard {
    fn drop(&mut self) {
        unsafe {
            std::env::remove_var("CHAINBOT_HTTP_NODE_ALLOW_LOOPBACK_FOR_TESTS");
            std::env::remove_var("CHAINBOT_INTERNAL_ALLOW_TEST_DESTINATIONS");
        }
    }
}
