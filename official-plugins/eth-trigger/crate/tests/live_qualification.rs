...
async fn live_qualification_subscribes_against_real_provider() {
    let endpoint = std::env::var("CHAINBOT_ETH_TRIGGER_LIVE_ENDPOINT")
        .expect("CHAINBOT_ETH_TRIGGER_LIVE_ENDPOINT must be set for live qualification");
    let allowed_origin = std::env::var("CHAINBOT_ETH_TRIGGER_LIVE_ALLOWED_ORIGIN")
        .unwrap_or_else(|_| endpoint.clone());
    let rpc_token = std::env::var("CHAINBOT_ETH_TRIGGER_LIVE_RPC_TOKEN").ok();
    ...
    let command = json!({
        ...
        "trigger_id": "eth-live-qualification",
        "source": "eth_log",
        "params": {
...
async fn handle_socket(
    ...
    if let Some(Ok(Message::Text(text))) = socket.next().await {
        ...
        let subscription_id = ack_payload["result"]
            .as_str()
            .expect("ack should include string subscription id");
        socket
            .send(Message::Text(
                ack_text
                    .into(),
