...
pub fn build_subscription_request(command: &TriggerStartCommand) -> Result<Value, String> {
    ...
    let coin = command
        ...
        .get("coin")
        .and_then(Value::as_str)
        .ok_or_else(|| String::from("params.coin is required"))?
        .trim();
    if coin.is_empty() {
        return Err(String::from("params.coin must not be empty"));
    }
...
pub fn source_channel(source: &str) -> Result<&'static str, String> {
    match source {
        "hyperliquid_trades" => Ok("trades"),
        "hyperliquid_l2_book" => Ok("l2Book"),
        other => Err(format!("unsupported Hyperliquid trigger source {other}")),
    }
}

fn validate_source(source: &str) -> Result<(), String> {
    let _ = source_channel(source)?;
    Ok(())
}

fn validate_coin(params: &BTreeMap<String, Value>) -> Result<(), String> {
    let coin = params
        .get("coin")
        .and_then(Value::as_str)
        .ok_or_else(|| String::from("params.coin is required"))?;
