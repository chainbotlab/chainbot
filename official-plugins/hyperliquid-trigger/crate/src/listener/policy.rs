...
pub fn validate_destination_policy(url: &str, allowed_origins: &[String], requires_binding: bool) -> Result<(), String> {
...
fn validate_scheme(url: &Url) -> Result<(), String> {
    if matches!(url.scheme(), "wss") {
        return Ok(());
    }
    if matches!(url.scheme(), "ws") && allow_loopback_for_tests() && is_loopback_host(url.host_str()) {
        return Ok(());
    }
    Err(String::from("destination url must use wss"))
}
