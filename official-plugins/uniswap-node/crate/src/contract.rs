...
pub fn metadata(protocol: &str, confirmation_mode: Option<&str>) -> Value {
    match confirmation_mode {
        Some(confirmation_mode) => {
            json!({"protocol": protocol, "confirmation_mode": confirmation_mode})
        }
...
mod tests {
    ...
    fn request() -> PluginRequest {
        PluginRequest {
            contract_version: String::from(CONTRACT_VERSION),
            plugin_id: String::from("uniswap-node"),
            node_id: String::from("node-1"),
            operation: String::from("uniswap_get_amounts_out"),
            input: BTreeMap::new(),
            activation: None,
        }
    }
