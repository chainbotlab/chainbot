...
use crate::contract::{build_event_frame, build_login_request, build_subscription_request, event_message, TriggerStartCommand};

use super::normalize::{current_time_ms, event_key, event_stream_name, event_time_ms, is_login_ack, is_subscription_ack, normalize_payload, stream_error};
...
pub async fn run_private_listener(command: &TriggerStartCommand, stdout: &mut impl Write) -> Result<(), String> {
    let endpoint = private_endpoint(command)?;
    let login_request = build_login_request(command)?;
    ...
    loop {
        ...
        socket
            .send(Message::Text(login_request.to_string().into()))
            .await
            .map_err(|error| error.to_string())?;

        match consume_socket_events(command, stdout, &mut socket, &subscription_request, &mut emitted_ready).await? {
...
async fn consume_socket_events(
    ...
    while let Some(message) = socket.next().await {
        ...
        match handle_socket_message(command, stdout, socket, message, subscription_request, &mut logged_in, emitted_ready).await? {
        ...
        Message::Text(text) => {
            ...
            if let Some((code, message)) = stream_error(&payload) {
                return Err(match code {
                    Some(code) => format!("okx private stream error [{code}] {message}"),
                    None => format!("okx private stream error {message}"),
                });
            }
            if !*logged_in {
                return Err(String::from("private stream emitted event before login acknowledgement"));
            }
...
fn private_endpoint(command: &TriggerStartCommand) -> Result<String, String> {
    ...
    Ok(default_private_ws_url(product_line, environment).to_owned())
}

fn default_private_ws_url(product_line: ProductLine, environment: Environment) -> &'static str {
    super::types::default_private_ws_url(product_line, environment)
}
