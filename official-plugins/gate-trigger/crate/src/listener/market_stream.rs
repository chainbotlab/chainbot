...
use super::normalize::{current_time_ms, current_time_secs, event_key, event_stream_name, event_time_ms, is_subscription_ack, normalize_payload, stream_error};
use super::policy::validate_destination_policy;
use super::types::{channel_from_params, payload_from_params, validate_market_channel, MessageOutcome, SocketLoopOutcome, DEFAULT_SPOT_WS_URL};
...
pub async fn run_market_listener(command: &TriggerStartCommand, stdout: &mut impl Write) -> Result<(), String> {
    ...
    validate_market_channel(&channel)?;
    ...
    loop {
        ...
        let subscribe = build_subscribe_request(&channel, payload.clone(), None)?;
        socket
    ...
    match message {
        Message::Text(text) => {
            ...
            if let Some(message) = stream_error(&payload) {
                return Err(format!("gate stream error {message}"));
            }
            ...
            if !matches!(payload.get("event").and_then(Value::as_str), Some("update")) {
                return Ok(MessageOutcome::Continue);
            }
...
fn build_subscribe_request(channel: &str, payload: Option<Value>, auth: Option<Value>) -> Result<Value, String> {
...
pub(super) fn market_endpoint(command: &TriggerStartCommand) -> Result<String, String> {
    ...
    Ok(String::from(DEFAULT_SPOT_WS_URL))
}

pub(super) fn user_subscribe_request(channel: &str, payload: Option<Value>, auth: Value) -> Result<Value, String> {
    build_subscribe_request(channel, payload, Some(auth))
}
