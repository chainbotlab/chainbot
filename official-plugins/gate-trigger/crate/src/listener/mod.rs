...
pub async fn run_listener_with_writer(command: TriggerStartCommand, stdout: &mut impl Write) -> Result<(), String> {
    ...
    if source != "gate_spot_market_stream" && source != "gate_spot_user_stream" {
        return Err(format!("unsupported Gate trigger source {source}"));
    }
    ...
    match source {
        "gate_spot_market_stream" => market_stream::run_market_listener(&command, stdout).await,
        "gate_spot_user_stream" => user_stream::run_user_stream_listener(&command, stdout).await,
        other => Err(format!("unsupported Gate trigger source {other}")),
    }
}
