...
use gate_trigger_official_plugin::{contract::parse_start_command, listener::run_listener_with_writer};
use serde_json::Value;

#[tokio::test]
#[serial_test::serial]
async fn gate_market_mock_listener_emits_event_frame() {
    let _loopback = LoopbackGuard::set();
    let command = serde_json::json!({
        "type": "start",
        "protocol_version": "2.0.0",
        "trigger_id": "gate-market-trigger",
        "source": "gate_spot_market_stream",
        "params": {
            "endpoint": "mock://gate-market",
            "channel": "spot.trades",
    ...
    run_listener_with_writer(parse_start_command(&command.to_string()).expect("valid start command"), &mut stdout)
        .await
        .expect("mock gate market listener should succeed");
    ...
    assert_eq!(frames[1]["payload"]["exchange"], "gate");
    assert_eq!(frames[1]["payload"]["listener_kind"], "market_stream");
    assert_eq!(frames[1]["payload"]["stream"], "spot.trades:BTC_USDT");
}

#[tokio::test]
#[serial_test::serial]
async fn gate_user_mock_listener_emits_event_frame() {
    let _loopback = LoopbackGuard::set();
    let command = serde_json::json!({
        "type": "start",
        "protocol_version": "2.0.0",
        "trigger_id": "gate-user-trigger",
        "source": "gate_spot_user_stream",
        "params": {
            "endpoint": "mock://gate-user",
            "channel": "spot.orders",
            "currency_pair": "BTC_USDT"
        },
        "activation": {
    ...
    run_listener_with_writer(parse_start_command(&command.to_string()).expect("valid start command"), &mut stdout)
        .await
        .expect("mock gate user listener should succeed");
    ...
    assert_eq!(frames[1]["payload"]["stream"], "spot.orders:BTC_USDT");
}
...
async fn unsupported_source_does_not_emit_ready() {
    let command = serde_json::json!({
        "type": "start",
        "protocol_version": "2.0.0",
        "trigger_id": "gate-unsupported-trigger",
        "source": "gate_unknown_stream",
        "params": {},
        "heartbeat_interval_ms": 1000,
        "shutdown_grace_ms": 1000
    });
    ...
    assert!(error.contains("unsupported Gate trigger source"));
...
async fn unsupported_protocol_version_does_not_emit_ready() {
    let command = serde_json::json!({
        ...
        "trigger_id": "gate-unsupported-protocol",
        "source": "gate_spot_market_stream",
        "params": {
            "endpoint": "mock://gate-market",
            "channel": "spot.trades",
...
fn insecure_market_endpoint_scheme_is_rejected() {
    let command = serde_json::json!({
        "type": "start",
        "protocol_version": "2.0.0",
        "trigger_id": "gate-insecure-endpoint",
        "source": "gate_spot_market_stream",
        "params": {
            "endpoint": "ws://example.com/ws/v4",
            "channel": "spot.trades",
            "currency_pair": "BTC_USDT"
        },
        "activation": {
