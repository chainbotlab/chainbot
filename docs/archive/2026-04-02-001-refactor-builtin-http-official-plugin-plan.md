...
- `docs/decisions/CHAINBOT_OFFICIAL_PLUGIN_DESIGN.md` 已经把 official plugin / thin runtime 边界定死：HTTP semantics 默认应 plugin-owned，而不是 runtime-owned。
- `docs/decisions/CHAINBOT_PLUGIN_ACTIVATION_CONFIG_DESIGN.md` 明确了 operator-owned secret 应走 `plugin_activation.<plugin_id>.secret_bindings`，并在 execution time 注入 `activation.secrets`。
- `docs/decisions/CHAINBOT_CONFIG_STATE_LAYOUT_DESIGN.md` 与 `docs/engineering/CHAINBOT_CONFIG_STATE_LAYOUT_IMPLEMENTATION.md` 表明 package identity 是 directory name + `plugin_id` 的硬边界，不能把这次迁移当成纯代码搬运。
- `docs/research/CHAINBOT_CLI_CATALOG_DISCOVERY_PROPOSAL.md` 和 `docs/decisions/CHAINBOT_CLI_DESIGN.md` 表明 discoverability 必须走 stable read model；HTTP 迁移后，catalog/source/install surface 需要同步闭环。
- `docs/engineering/CHAINBOT_CORE_BUILTIN_NODES_IMPLEMENTATION.md` 的 builtin philosophy 已倾向把 heavier integration surface 留在 plugin side，这次迁移与该方向一致。
...
- If implementation details are worth preserving, add an implementation record under `docs/engineering/` rather than mutating stable design docs into historical logs.
...
- Related docs: `docs/decisions/CHAINBOT_CONFIG_STATE_LAYOUT_DESIGN.md`
- Related docs: `docs/research/CHAINBOT_CLI_CATALOG_DISCOVERY_PROPOSAL.md`
- Related plans: `docs/archive/2026-03-31-001-feat-eth-solana-official-plugins-plan.md`
- Related plans: `docs/archive/2026-04-01-001-feat-eth-solana-sdk-plugins-plan.md`
